...
const DateSelectionCalendar: React.FC<DateSelectionCalendarProps> = ({ onSelectDate, rooms, bookings }) => {
      ...
      days.push(
        <button
          ...
          className={`h-14 md:h-24 border border-subtle flex flex-col items-start justify-between p-1 md:p-2 transition-all relative
            ${isPast ? 'bg-main text-secondary opacity-50 cursor-not-allowed' : 'hover:bg-brand/10 cursor-pointer bg-surface'}
        ...
        >
          ...
          {!isPast && isNext7Days && availableCount !== null && (
