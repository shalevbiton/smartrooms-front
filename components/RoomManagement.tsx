    ...
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {filteredRooms.map((room) => (
          <div 
            key={room.id} 
            className={`group relative bg-surface rounded-[2rem] border transition-all duration-300 flex flex-col overflow-hidden hover:shadow-2xl hover:-translate-y-1 ${
              !room.isAvailable ? 'border-red-500/20' : 'border-subtle shadow-sm'
            }`}
          >
            {/* Image Section */}
