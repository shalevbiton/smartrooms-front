...
import { 
  ...
  Users as UsersLucide, Info, Eye, EyeOff, FileSpreadsheet 
} from 'lucide-react';
import { User } from '../types';
import { downloadFile, convertUsersToCSV } from '../utils/downloadUtils';
...
const UserManagement: React.FC<UserManagementProps> = ({ 
  ...
  const handleExportUsers = () => {
    ...
    downloadFile(csv, `users_list_${dateStr}.csv`, 'text/csv;charset=utf-8;');
  };
    ...
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-10">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto">
          <button 
            ...
            className="flex items-center justify-center gap-2 px-6 py-3.5 bg-surface border border-subtle rounded-2xl text-xs font-black text-secondary hover:text-brand hover:border-brand transition-all shadow-sm active:scale-95"
          >
            <FileSpreadsheet size={16} /> ייצוא רשימה
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredUsers.map((user) => (
          <div 
            key={user.id} 
            className={`relative group bg-surface rounded-3xl border transition-all duration-300 flex flex-col overflow-hidden hover:shadow-xl hover:-translate-y-1 ${
              user.status === 'PENDING' 
              ...
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-white font-bold shrink-0 shadow-inner border transition-transform group-hover:scale-105 ${
                      user.role === 'ADMIN' ? 'bg-brand border-brand/20' : 'bg-tertiary border-subtle'
                    }`}>
                      {user.avatar ? (
